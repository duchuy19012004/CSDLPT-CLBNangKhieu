-- =============================================
-- HỆ THỐNG QUẢN LÝ CÂU LẠC BỘ - PHÂN MẢNH THEO ID (CHẴN/LẺ)
-- Phân mảnh dựa trên giá trị ID (ID-based Fragmentation)
-- 
-- LOGIC PHÂN MẢNH:
-- - MaCLB lẻ (1, 3, 5, 7...) → Site A
-- - MaCLB chẵn (2, 4, 6, 8...) → Site B
-- - Các bảng liên quan phân theo MaCLB của CauLacBo
-- - Phân bố đều 50-50, dễ scale, dễ hiểu
-- =============================================
...
('FragmentationType', 'ID_BASED', N'Phân mảnh theo ID chẵn/lẻ'),
('SiteA_Rule', 'ODD', N'Site A chứa ID lẻ (1, 3, 5, 7...)'),
('SiteB_Rule', 'EVEN', N'Site B chứa ID chẵn (2, 4, 6, 8...)');
GO

-- Function xác định site theo MaCLB
IF OBJECT_ID('dbo.GetSiteByMaCLB', 'FN') IS NOT NULL DROP FUNCTION dbo.GetSiteByMaCLB;
GO

CREATE FUNCTION dbo.GetSiteByMaCLB(@MaCLB INT)
RETURNS VARCHAR(10)
AS
BEGIN
    -- Lẻ → SiteA, Chẵn → SiteB
    RETURN CASE WHEN @MaCLB % 2 = 1 THEN 'SiteA' ELSE 'SiteB' END;
END;
GO

-- =============================================
-- SITE A: Tạo bảng (chứa ID lẻ)
-- =============================================
USE SiteA;
GO
...
CREATE TABLE CauLacBo (
    ...
    CONSTRAINT CHK_SiteA_OddID CHECK (MaCLB % 2 = 1) -- Chỉ nhận ID lẻ
);
...
CREATE TABLE CauLacBo (
    ...
    TenCLB NVARCHAR(100) NOT NULL,
    TenKhoa NVARCHAR(50) NOT NULL,
    CONSTRAINT CHK_SiteB_EvenID CHECK (MaCLB % 2 = 0) -- Chỉ nhận ID chẵn
);
...
-- =============================================
-- TRIGGER PHÂN MẢNH THEO ID CHẴN/LẺ
-- =============================================
...
BEGIN
    ...
    SELECT MaCLB, TenCLB, TenKhoa FROM inserted WHERE MaCLB % 2 = 1;
    
    -- ID chẵn → Site B
    INSERT INTO SiteB.dbo.CauLacBo (MaCLB, TenCLB, TenKhoa)
    SELECT MaCLB, TenCLB, TenKhoa FROM inserted WHERE MaCLB % 2 = 0;
END;
GO
...
-- Trigger INSERT cho GiangVien (theo MaCLB)
...
BEGIN
    ...
    -- GV thuộc CLB có ID lẻ → Site A
    INSERT INTO SiteA.dbo.GiangVien (MaGV, HoTenGV, MaCLB)
    SELECT MaGV, HoTenGV, MaCLB FROM inserted WHERE MaCLB % 2 = 1;
    
    -- GV thuộc CLB có ID chẵn → Site B
    INSERT INTO SiteB.dbo.GiangVien (MaGV, HoTenGV, MaCLB)
    SELECT MaGV, HoTenGV, MaCLB FROM inserted WHERE MaCLB % 2 = 0;
END;
GO
...
-- Trigger INSERT cho SinhVien (theo MaCLB)
...
BEGIN
    ...
    SELECT MaSV, HoTenSV, MaCLB FROM inserted WHERE MaCLB % 2 = 1;
    
    INSERT INTO SiteB.dbo.SinhVien (MaSV, HoTenSV, MaCLB)
    SELECT MaSV, HoTenSV, MaCLB FROM inserted WHERE MaCLB % 2 = 0;
END;
GO
...
-- Trigger INSERT cho LopNangKhieu (theo site của GiangVien)
...
-- Trigger INSERT cho BienLai (theo site của LopNangKhieu)
...
-- Site A: CLB có ID lẻ (1, 3, 5)
USE SiteA;
GO

INSERT INTO CauLacBo VALUES (1, N'Câu lạc bộ Tin học', N'K1');
INSERT INTO CauLacBo VALUES (3, N'Câu lạc bộ Toán học', N'K2');
INSERT INTO CauLacBo VALUES (5, N'Câu lạc bộ Văn học', N'K1');

INSERT INTO GiangVien VALUES ('GV1', N'Nguyễn Thị Hương', 1);
INSERT INTO GiangVien VALUES ('GV3', N'Trần Văn Minh', 3);
INSERT INTO GiangVien VALUES ('GV5', N'Lê Thị Mai', 5);

INSERT INTO SinhVien VALUES ('SV001', N'Phạm Quốc Anh', 1);
INSERT INTO SinhVien VALUES ('SV003', N'Hoàng Thị Lan', 3);
INSERT INTO SinhVien VALUES ('SV005', N'Đỗ Văn Hùng', 5);

INSERT INTO LopNangKhieu VALUES (1, '2024-08-01', 'GV1', 500000);
INSERT INTO LopNangKhieu VALUES (3, '2024-08-15', 'GV3', 600000);
INSERT INTO LopNangKhieu VALUES (5, '2024-09-01', 'GV5', 550000);

INSERT INTO BienLai VALUES (1, 8, 2024, 1, 'SV001', 500000);
INSERT INTO BienLai VALUES (3, 8, 2024, 3, 'SV003', 600000);
INSERT INTO BienLai VALUES (5, 9, 2024, 5, 'SV005', 550000);
GO

-- Site B: CLB có ID chẵn (2, 4, 6)
USE SiteB;
GO

INSERT INTO CauLacBo VALUES (2, N'Câu lạc bộ Anh văn', N'K3');
INSERT INTO CauLacBo VALUES (4, N'Câu lạc bộ Thể thao', N'K4');
INSERT INTO CauLacBo VALUES (6, N'Câu lạc bộ Âm nhạc', N'K5');

INSERT INTO GiangVien VALUES ('GV2', N'Võ Thị Ngọc', 2);
INSERT INTO GiangVien VALUES ('GV4', N'Đặng Minh Tuấn', 4);
INSERT INTO GiangVien VALUES ('GV6', N'Bùi Thị Phương', 6);

INSERT INTO SinhVien VALUES ('SV002', N'Trương Văn Nam', 2);
INSERT INTO SinhVien VALUES ('SV004', N'Lý Thị Hà', 4);
INSERT INTO SinhVien VALUES ('SV006', N'Phan Minh Khoa', 6);

INSERT INTO LopNangKhieu VALUES (2, '2024-08-20', 'GV2', 700000);
INSERT INTO LopNangKhieu VALUES (4, '2024-07-01', 'GV4', 650000);
INSERT INTO LopNangKhieu VALUES (6, '2024-09-10', 'GV6', 800000);

INSERT INTO BienLai VALUES (2, 8, 2024, 2, 'SV002', 700000);
INSERT INTO BienLai VALUES (4, 7, 2024, 4, 'SV004', 650000);
INSERT INTO BienLai VALUES (6, 9, 2024, 6, 'SV006', 800000);
GO

-- =============================================
-- ACTIVITY LOG (tùy chọn)
-- =============================================
USE SiteA;
GO
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM inserted) AND NOT EXISTS (SELECT 1 FROM deleted)
        ...
        SELECT 'INSERT', 'CauLacBo', CAST(MaCLB AS NVARCHAR), 'SiteA', N'Thêm CLB: ' + TenCLB FROM inserted;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM inserted) AND NOT EXISTS (SELECT 1 FROM deleted)
        INSERT INTO ActivityLog (Action, TableName, RecordId, Site, Details)
        SELECT 'INSERT', 'CauLacBo', CAST(MaCLB AS NVARCHAR), 'SiteB', N'Thêm CLB: ' + TenCLB FROM inserted;
    ELSE IF EXISTS (SELECT 1 FROM inserted) AND EXISTS (SELECT 1 FROM deleted)
        INSERT INTO ActivityLog (Action, TableName, RecordId, Site, Details)
...
PRINT N'========================================';
PRINT N'HOÀN THÀNH SETUP - PHÂN MẢNH THEO ID';
PRINT N'';
PRINT N'LOGIC PHÂN MẢNH:';
PRINT N'- MaCLB lẻ (1,3,5...) → Site A';
PRINT N'- MaCLB chẵn (2,4,6...) → Site B';
PRINT N'- GiangVien, SinhVien: theo MaCLB';
PRINT N'- LopNangKhieu: theo MaGV';
PRINT N'- BienLai: theo MaLop';
PRINT N'';
PRINT N'ƯU ĐIỂM:';
PRINT N'- Phân bố đều 50-50';
PRINT N'- Dễ scale, dễ hiểu';
PRINT N'- Không phụ thuộc nghiệp vụ';
PRINT N'========================================';
...
PRINT N'Đã thêm trigger log cho tất cả các bảng!';
